...
Maze::~Maze() { delete this->poMazeData; }
...
bool Maze::canMove(Position pos, Direction dir) {
  ...
  case Direction::North:
    if (pos.row == 0) {
      ...
      status = ((getCell(pos.move(Direction::North)) &
                 (int)InternalBit::SOUTH_BIT) == 0);
    }
    break;
  ...
  case Direction::West:
    if (pos.col == 0) {
      ...
      status = ((getCell(pos.move(Direction::West)) &
                 (int)InternalBit::EAST_BIT) == 0);
    }
    break;
...
void Maze::Load(const std::string &fileName) {
  ...
  memset(this->poMazeData, 0x0,
         (this->width * this->height) * sizeof(unsigned int));
...
bool Maze::checkSolution(std::vector<Direction> &soln) {
  ...
  if (results) {
    std::cout << "    checkSolution(" << (int)soln.size() << " "
              << "elements):" << " passed\n";
